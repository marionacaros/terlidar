...
def train(
    ...
    # tiles = set([path_f.split('_')[-2].split('.')[0] for path_f in  train_files])
    # tiles = list(tiles)
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset,
                                                 ...
                                                 drop_last=True)
    # models
    pointnet = PointNet2(num_classes=num_classes, num_feat=num_feat, radius=[0.1, 0.2, 0.4, 0.8])
    pointnet.to(device)
    ...
    if model_checkpoint:
        ...
        print('Fine-tunning')
        pointnet.load_state_dict(checkpoint['model'], strict=False) # strict=True when loading all weights
        # optimizer.load_state_dict(checkpoint['optimizer']) # COMMENT WHEN FINE-TUNING
    ...
    for epoch in progressbar(range(epochs)):
        ...
        for data in train_dataloader:
            ...
            # Segmentation labels:
            # iou['ground_train'].append(get_iou_obj(targets, preds, 1))
            # iou['surr_train'].append(get_iou_obj(targets, preds, 0))
            # iou['powerlines_train'].append(get_iou_obj(targets, preds, 3))
            # iou['poles_train'].append(get_iou_obj(targets, preds, 2))
        ...
        with torch.no_grad():
            for data in val_dataloader:
                ...
                # Add IoU for all categories
                # iou['ground_val'].append(get_iou_obj(targets, preds, 1))
                # iou['surr_val'].append(get_iou_obj(targets, preds, 0))
                # iou['powerlines_val'].append(get_iou_obj(targets, preds, 3))
                # iou['poles_val'].append(get_iou_obj(targets, preds, 2))
        ...
        # for category in ['ground', 'powerlines', 'veg', 'fences', 'buildings', 'cars', 'trucks','poles']: 
        for category in ['ground', 'powerlines', 'poles', 'veg', 'buildings', 'cars']: 
