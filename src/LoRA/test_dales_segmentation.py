...
def test(output_dir,
    ...
    with torch.no_grad():
        for i, data in enumerate(progressbar(test_dataloader, )):
            ...
            ids = ids.squeeze(0).cpu().to(torch.int64)
            ...
            if ids.shape[0] != targets.shape[0]:
                print('Error: ids and targets have different shapes')
            ...
            labels_tile[ids.numpy()] = targets_1hot

            pc = pointcloud.squeeze(0).to(DEVICE)  # [n_samp, n_points, dims]
            dims = pc.shape[2]

            if n_points < pc.shape[0] :
                # ------------- duplicate points with different samplings ----------------
                pc2, ids2, _ = get_sampled_sequence(pc.view(-1, dims), ids, n_points)
                pc3, ids3, _ = get_sampled_sequence(pc.view(-1, dims), ids, n_points)
                pc = torch.cat((pc, pc2, pc3), dim=0)
                ids = torch.cat((ids, ids2, ids3), dim=0)
                # ------------------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_points', type=int, default=16000, help='number of points per point cloud')
    ...
    with open(file_path, 'a') as fid:
        ...
        if os.stat(file_path).st_size == 0:
            ...
            # writer.writerow(["Tile", 'ground iou', 'powerline_poles iou', 'veg iou', 'fences iou', 'buildings iou', 'cars_trucks iou', 'mean iou','pts. ground', 'pts. powerline_poles', 'pts. veg', 'pts. fences','pts. buildings', 'pts. cars_trucks'])
            writer.writerow(["Tile", 'surrounding iou', 'ground iou', 'poles iou', 'powerlines iou', 'mean iou', 'pts. surrounding', 'pts. ground', 'pts. poles', 'pts. powerlines'])
    ...
    # tiles=['54435', '54440', '54445', '54470', '54490']
    ...
    for tile in tiles:
        ...
        categories = ['surrounding', 'ground',  'poles', 'powerlines']
